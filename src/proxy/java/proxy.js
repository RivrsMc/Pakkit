...
capabilities.versionId = 'java-node-minecraft-protocol-' + version.split('.').join('-')

const mcdata = minecraft_data(version) // Used to get packets, may remove if I find a better way
...
capabilities.clientboundPackets = mcdata.protocol.play.toClient.types.packet[1][0].type[1].mappings
capabilities.serverboundPackets = mcdata.protocol.play.toServer.types.packet[1][0].type[1].mappings
...
if (taken) {
  ...
  srv.on('login', function (client) {
    ...
    client.on('end', function () {
      ...
      if (!endedTargetClient) { end('End') }
    })
    client.on('error', function (err) {
      endedClient = true
      console.log('Connection error by client', '(' + addr + ')')
      console.log(err.stack)
      if (!endedTargetClient) { end('Error') }
    })
    // if (authConsent) {
    ...
    targetClient.on('end', function () {
      ...
      if (!endedClient) { end('Connection closed by server ' + '(' + host + ':' + port + ')') }
    })
    targetClient.on('error', function (err) {
      ...
      messageCallback(header, message)
      if (!endedClient) { end('pakkit - ' + header + '\n' + message) }
    })
  })
}
})
